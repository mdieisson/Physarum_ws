...
def teletransportar_container(set_pose_proxy, base_pose, offset=-0.5):
    x, y, theta = base_pose
...
def main():
    ...
    rospy.init_node("controller_multirobot_com_container")
    ...
    # Escolhe ponto de coleta mais próximo
    ponto = min(pontos_coleta, key=lambda p: euclidean(robot_pose[:2], p))
    rospy.loginfo(f"{robot_name}: indo para ponto de coleta {ponto}")
    fase = 'indo_para_coleta'
    enviar_meta(client, *ponto)
    chegou = aguardar_chegada(client, ponto)

    if chegou:
        container_name = lista_containers.pop
    ...
    while not rospy.is_shutdown():
        if container_acoplado and robot_pose:
            teletransportar_container(set_pose_proxy, robot_pose)
        rate.sleep()
